...
export async function registerAction(input: RegisterInput): Promise<ActionResult> {
  ...
  // This is a single-tenant instance: only the first account may self-register
  // (as the TUTOR/owner). Afterwards, accounts are created from within the app.
  ...
  if (error || !user) {
    ...
    return fail("Не удалось создать аккаунт. Попробуйте ещё раз.");
  }

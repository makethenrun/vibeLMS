...
export function SentenceSolve({ itemId, content, initialScore }: Props) {
  ...
  switch (content.variant) {
    case "WORD_ORDER":
      return <div className="space-y-2">{prompt}<OrderSolve itemId={itemId} tokens={content.words} initialScore={initialScore} /></div>;
    case "SENTENCE_ORDER":
      return <div className="space-y-2">{prompt}<OrderSolve itemId={itemId} tokens={content.sentences} initialScore={initialScore} /></div>;
    case "WORD_FROM_LETTERS":
      return <div className="space-y-2">{prompt}<WordLettersSolve itemId={itemId} word={content.word} extraLetters={content.extraLetters} initialScore={initialScore} /></div>;
    case "SORT_COLUMNS":
      return <div className="space-y-2">{prompt}<SortColumnsSolve itemId={itemId} content={content} initialScore={initialScore} /></div>;
    case "MATCH_PAIRS":
      return <div className="space-y-2">{prompt}<MatchPairsSolve itemId={itemId} pairs={content.pairs} initialScore={initialScore} /></div>;
  }
}

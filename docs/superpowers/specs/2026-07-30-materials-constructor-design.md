...
## Ревизия 2026-08-01 (7) — Фаза 4: работа с предложениями

Новый тип `SENTENCE_TASK` (миграция `0013_sentence_task_item_type.sql`) с полем
`variant`, 5 вариаций (авторинг-конфиг; интерактив — в плеере):
- `WORD_ORDER` — расставить слова в правильном порядке (`words[]`);
- `SORT_COLUMNS` — сортировка по колонкам (`columns[{title, items[]}]`);
- `SENTENCE_ORDER` — расставить предложения по порядку (`sentences[]`);
- `WORD_FROM_LETTERS` — составить слово из букв (`word` + `extraLetters`);
- `MATCH_PAIRS` — сопоставить слова (`pairs[{left,right}]`).

«Сопоставление пар» (тип `MATCH`) убрано из меню добавления и стало вариацией
`MATCH_PAIRS`. Тип `MATCH` остаётся в enum и рендерится для ранее созданных
элементов (обратная совместимость). Один редактор `sentence-task-editor.tsx`.

### БД на бою
После `0012` накатить `0013_sentence_task_item_type.sql`.

На этом расширение форматов упражнений завершено (Фазы 1–4). Следующий крупный
срез — **студенческий плеер**, чтобы форматы стали проходимыми (drag/выбор/ввод,
таймер+blur, проверка и баллы).

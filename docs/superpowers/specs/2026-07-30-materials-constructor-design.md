...
Vitest (уже настроен, `vitest.config.ts`). Покрыть:
- zod-схемы `content` каждого типа (валидные/невалидные кейсы, discriminated
  union по `type`).
- Логику ре-ордера позиций в сервисах (перестановка соседей, границы списка).
- CRUD-сервисы уровней (с моками `Db`, по образцу существующих тестов).

По образцу существующих тестов проекта.
